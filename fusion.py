...
def get_fusion_dict():
    ...
    config_data = config["FUSIONS"]
    try:
        fusions = config_data['fusions'].split('@')
        for fusion in fusions:
            labels_list, new_label = fusion.split(':')
            if labels_list.strip()=="$OTHER$":

                labels=list(set(names.values()).difference(set(dico_fusions.keys())))
            else:
                labels = labels_list.split(',')
            
            for label in labels:
                dico_fusions[label.strip()] = new_label.strip()
        return dico_fusions
